...
class WebGLMesh extends Mesh{
    constructor (gl,program,color,filename,alias,attributes,texturename){
        super(color,filename, alias, attributes);
        this.gl = gl;
        this.program = program;
        this.texturename = texturename;
    ...
    async fetch(){
        await this.load(this.filename,this.alias,this.attributes);
    }

    getWebGLModel(attribShaderVariables = null,uniformShaderVariables = null){
              
        const illumAttrib = {"matAmbient":this.object.ambient,
                             "Ka":this.object.Ka,
                             "matDiffuse":this.object.diffuse,
                             "Kd":this.object.Kd,
                             "matSpecular":this.object.specular,
                             "Ks":this.object.Ks,
                             "Ns":this.object.Ns,
                             "D":this.object.d};

        if (this.object.normals === undefined){
            this.object.normals = utils.calculateNormals(this.object.vertices,this.object.indices);
        }
        
        GeomUtils.computeCenter(this.object);

        const webGLMeshModel = new WebGLMeshModel(this.gl,this.program,3,this.gl.TRIANGLES,
                                              this.object.vertices,this.object.indices,this.object.colors,this.object.normals,
                                              null,this.object.textureCoords,this.textureName,
                                              illumAttrib);

        webGLMeshModel.set(attribShaderVariables,uniformShaderVariables);

        return webGLMeshModel;
    }
}
